...
void adc_init(void)
{
	...
	RCC_APB2PeriphResetCmd(RCC_APB2Periph_ADC1 | RCC_APB2Periph_ADC2, ENABLE);  //ADC复位
	...
	ADC_InitStructure.ADC_NbrOfConversion = 1;                                 //12个转换在规则序列中
	...
	//ADC_RegularChannelConfig(ADC2, ADC_Channel_10, 2, ADC_SampleTime_84Cycles );
	...
	DMA_config(DMA2_Stream3, DMA_Channel_1, (uint32_t)&ADC2->DR, (uint32_t)adc2_buf, 1000);
...
void adc_test_init(void)
{
	...
	//ADC_RegularChannelConfig(ADC2, ADC_Channel_11, 2, ADC_SampleTime_84Cycles );
...
uint16_t *get_work_current_adc(uint16_t num)
{
	...
	while(DMA_GetFlagStatus(DMA2_Stream3, DMA_FLAG_TCIF3) == RESET && i < 1000000)//等待DMA2_Steam0传输完成
		i++;
	if (i >= 1000000) {
