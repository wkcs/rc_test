...
//addr:flash地址
//返回值:0~11,即addr所在的扇区
uint16_t flash_get_flash_sector(u32 addr)
{
  ...
  FLASH_DataCacheCmd(DISABLE);//FLASH擦除期间,必须禁止数据缓存
	...
	if(addrx<0X1FFF0000)			//只有主存储区,才需要执行擦除操作!!
	{
		while(addrx<endaddr)		//扫清一切障碍.(对非FFFFFFFF的地方,先擦除)
		{
			...
			{   
				status=FLASH_EraseSector(flash_get_flash_sector(addrx),VoltageRange_3);//VCC=2.7~3.6V之间!!
...
void flash_read(u32 ReadAddr,u32 *pBuffer,u32 NumToRead)   	
{
	...
	{
		pBuffer[i]=flash_read_word(ReadAddr);//读取4个字节.
		ReadAddr+=4;//偏移4个字节.	
	}
}
